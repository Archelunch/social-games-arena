# Backlog — Social Deduction Benchmark

Durable task list. Derived from `social-deduction-benchmarks/WEREWOLF_DESIGN.md`.
Status: `[ ]` todo · `[~]` in progress · `[x]` done · `[!]` blocked.
Pick the next task whose dependencies are all `[x]`. Update this file when a task changes state.

---

## M1 — Project scaffold & engine core

The game-agnostic, pure, seeded state machine. No game rules here.

- [x] **T01** — Scaffold `pyproject.toml` (Poetry, py3.13; deps: `dspy`, `litellm`, `trueskill`; dev: `pytest`, `ruff`, `pyrefly`), `src/social_deduction_bench/` package layout, `.gitignore` (`.reviews/`, `.dspy_cache/`, caches), `[tool.ruff]` + `[tool.pyrefly]` config. _Depends: —_
- [x] **T02** — Seeded RNG utility: a `Random` threaded explicitly through the engine; no global random state. _Depends: T01_
- [x] **T03** — Game state model: players, roles, alive/dead, round, current phase. Frozen/structured types. _Depends: T01_
- [x] **T04** — Event stream: append-only event log + JSONL serialization (write + read back). _Depends: T03_
- [x] **T05** — Observation routing: public events to all, private events to one agent; agents never see hidden state. _Depends: T04_
- [x] **T06** — Phase state machine: night/day transitions, terminal detection hook. _Depends: T03_
- [x] **T07** — Tool-call validation: reject illegal moves (dead target, wrong phase/role) with an informative error observation. _Depends: T05, T06_
- [x] **T08** — Determinism harness: test utility asserting same seed → identical event stream. _Depends: T04_

## M2 — Werewolf game rules

The Werewolf-specific rules plugged into the M1 engine.

- [x] **T09** — Role definitions (Werewolf, Seer, Doctor, Villager) + 7-player default config. The game definition also **declares its private event types** (`seer_inspect`, `werewolf_chat`, `doctor_protect`) for the engine's private-event guard — see Notes. _Depends: T03_
- [x] **T10** — Seeded role assignment. _Depends: T02, T09_
- [x] **T11** — Night resolution: werewolf joint kill vote, seer inspect, doctor protect, protection suppresses the kill. Private night events carry non-empty `recipients`; the engine **rejects a declared-private event type emitted with empty `recipients`** — see Notes. _Depends: T06, T07, T10_
- [x] **T12** — Day resolution: exile vote, majority rule, tie → no exile. _Depends: T06, T07_
- [x] **T13** — Win-condition checks: villagers win when both werewolves dead; werewolves win at parity. Checked after night AND after exile. _Depends: T11, T12_
- [x] **T14** — Full game-loop integration test: a scripted 7-player game runs to a terminal state deterministically. _Depends: T08, T13_

## M3 — Tool set

The tools the ReAct agent calls. See WEREWOLF_DESIGN.md §6.

- [x] **T15** — Game-action tools: `werewolf_chat`, `submit_kill_vote`, `seer_inspect`, `doctor_protect`, `submit_bid`, `speak`, `submit_exile_vote`. _Depends: T11, T12_
- [x] **T16** — Cognitive tools: `get_public_state`, `get_private_info`, `recall`, `remember`, `get_beliefs`, `set_belief`, `get_plan`, `set_plan`. _Depends: T18, T19_
- [x] **T17** — Tool/role/phase gating: each tool exposed only to the allowed role in the allowed phase. _Depends: T15_
- [x] **T18** — Bidding-based speech ordering: collect bids, top-K speak in bid order. _Depends: T15_

## M4 — DSPy ReAct agent & memory

- [x] **T19** — `GameMemory` (Tier 0): events, notes, beliefs; `remember`/`recall`/`set_belief`. _Depends: T01_
- [x] **T20** — Belief table + persistent plan string wired into `GameMemory`. _Depends: T19_
- [x] **T21** — DSPy ReAct agent: one decision-point loop, cognitive tools as intermediate steps, one game-action tool terminates the loop. _Depends: T16, T19_
- [x] **T22** — multi-model config: seat different LLMs as different players. _Depends: T21_
- [x] **T23** — Smoke game with real LLM agents (auto-skips when no API key). _Depends: T14, T22_

## M5 — Observability & analysis

Rich data capture so a human can debug *why* an agent acted a given way, and an audience can watch the game unfold. Three layered tasks: engine-side event expansion, agent-side trajectory sidecar, then the UI that consumes both. This milestone is sequenced **before** rating / tournament because (a) honest metrics in M6 (illegal-move rate, tokens) consume signals introduced here, and (b) without the audience-facing replay the project has no felt feedback that a real game just ran. See Notes for the invariant-preserving design (sidecar vs. event log).

- [x] **T29** — Dialogue, ballot attribution, and rejected-call events. Wire `werewolf_chat` into the night loop and `submit_bid` + `resolve_discussion` + `speak` into the day loop. Emit `WEREWOLF_CHAT` (private → living pack), `BID` (private → engine, surfaced as part of `DISCUSSION_RESOLVED`), `DISCUSSION_RESOLVED` (public — chosen speakers in order), `SPEECH` (public — speaker + text). Extend `kill_resolved` / `exile_resolved` payloads with the full ballot map (`{voter: target}`) so per-player attribution survives in the transcript. Emit a private `TOOL_REJECTED` event (payload `{tool, args, reason}`) addressed to the caller when `validate_tool_call` rejects an invocation inside a ReAct loop — closes the T17 / T19 / T21 carry-over and is the source signal T24's illegal-move-rate metric will consume. _Depends: T18, T21, T22_
- [x] **T30** — Per-decision trajectory + LM metadata sidecar `<game_id>.trajectories.jsonl` written next to `events.jsonl`. One line per decision-point ReAct loop: `{decision_seq, round, phase, caller, role, terminal_tool, committed_value, react_trajectory: [{iter, thought, tool, args, observation}, ...], lm_calls: [{model, prompt_tokens, completion_tokens, latency_ms, cost_usd, cached}, ...]}`. Captured automatically inside `ReActDecisionSource`; emitted by `run_game` next to the event log. The engine event log (invariant #5) is untouched — sidecar carries agent-internal signals that aren't part of the engine's source-of-truth contract. _Depends: T21, T22_
- [ ] **T31** — Replay UI for audience and behavior analysis. A notebook or static HTML viewer that consumes `events.jsonl` + `trajectories.jsonl` and renders, round by round: alive/dead board state, public dialogue, per-player ballots, the seer/doctor private channel (when replaying as them or as "spectator"), each agent's ReAct thoughts and committed tool, and per-call cost / tokens / latency. Audience-facing — works as a standalone HTML page. _Depends: T29, T30_

## M6 — Rating, metrics & tournament

- [x] **T24** — Metric extraction from the event stream: win/loss, per-role win rate, illegal-move rate, game length, tokens. Adds a per-run `manifest.json` (seat→model provenance) consumed for cross-model rating. _Depends: T04, T14, T29, T30_
- [x] **T25** — Deceiver-vs-detector split metric (werewolf win rate vs. villager exile accuracy). _Depends: T24_
- [x] **T26** — TrueSkill rating: update individual ratings from team outcomes. _Depends: T24_
- [x] **T27** — Tournament runner: seeded cross-play over many games, ratings aggregated. _Depends: T23, T26_
- [x] **T28** — Leaderboard output + replay tool (event stream → deterministic re-run). _Depends: T08, T27_ — **leaderboard/insights site shipped** (`sdb-site`); the deterministic re-run verifier is deferred (see Notes).

---

## Notes

- M1 is fully game-agnostic; M2 is the first game plugged in. ONUW and Secret Hitler reuse M1/M3-M5.
- Open design questions (discussion slot count `K`, exile tie-break, cross-game memory) are parked in WEREWOLF_DESIGN.md §12 — resolve before the task that needs them, not earlier.
- **Private-event guard (cross-game).** An `Event` is private exactly when its `recipients` set is non-empty; "public" is the empty set. The game-agnostic engine cannot tell a broadcast from a private event that forgot its recipients, since it sees `type` only as an opaque string. So each game **declares its private event types** (T09) and the engine **rejects a declared-private type emitted with empty `recipients`** (enforced in T11). One shared M1 check, reused by Werewolf / ONUW / Secret Hitler — not re-coded per game. Rationale: WEREWOLF_DESIGN.md §3. Origin: integrity-review High on the T04 multi-recipient `recipients` change (2026-05-18).
- **Observability split (T29 / T30, M5).** Engine-side signals (ballots, dialogue, rejected calls) extend the event vocabulary and stay in the append-only event log — they're part of the game's source-of-truth. Agent-side signals (ReAct thoughts, LM call metadata, token cost) go in a sidecar JSONL file next to the event log — they're agent-internal and don't belong to the engine's referee contract. Invariants #5 (append-only event stream) and #1 (engine as single source of truth) stay intact; the visualization joins the two files by `(round, phase, caller)`.
- **T28 scope (2026-05-23).** The shipped deliverable is the static results site (`sdb-site`): a deterministic `data.json` re-aggregated from a run dir + a "Dusk dashboard" front-end (leaderboard with μ±3σ whiskers, deceiver-vs-detector quadrant scatter, head-to-head heatmap, cost/efficiency, self-play). **Deferred from T28's wording:** the deterministic event-stream *re-run verifier* (re-run a recorded game from its seed and assert an identical stream) — a small standalone tool, not part of the site; track as a follow-up. The visual *match replay* is T31 (the `data.json` games index is shaped to link to it later).
